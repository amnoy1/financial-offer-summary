...
export default function RecordPage() {
  ...
  async function submitText() {
    ...
    setStep('uploading')
    try {
      const res = await fetch('/api/meetings/from-text', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
  ...
  if (step === 'error') return (
    <div className="min-h-screen flex flex-col items-center justify-center px-6 bg-gray-50" dir="rtl">
      ...
      <button onClick={() => { setStep('setup'); setError('') }} className="mt-6 px-6 py-2 bg-blue-600 text-white rounded-full text-sm">נסה שוב</button>
    </div>
  )

  if (step !== 'setup' && step !== 'recording') return (
    <div className="min-h-screen flex flex-col items-center justify-center px-6 bg-gray-50" dir="rtl">
      <p className="text-5xl mb-6">{mode === 'text' ? '🤖' : '⏳'}</p>
      <h2 className="text-xl font-bold mb-8 text-gray-800">{mode === 'text' ? 'שולח לניתוח Claude...' : 'מעבד הקלטה...'}</h2>
      {mode !== 'text' && (
  ...
  if (step === 'recording') return (
    <div className="min-h-screen flex flex-col items-center justify-center px-6 bg-gray-50" dir="rtl">
      ...
      <div className="relative mb-10">
        ...
        <button onClick={stopRecording} className="relative w-24 h-24 rounded-full bg-red-500 flex items-center justify-center shadow-lg active:scale-95 transition-transform">
  ...
  return (
    <div className="min-h-screen bg-gray-50" dir="rtl">
      ...
      <main className="max-w-md mx-auto px-4 py-6 space-y-5">
          ...
          {showSuggestions && filteredClients.length > 0 && (
            <ul className="absolute z-10 w-full bg-white border border-gray-200 rounded-xl mt-1 shadow-lg overflow-hidden">
              {filteredClients.map(c => (
                <li key={c.id} onMouseDown={() => { setClientName(c.name); setClientId(c.id); setShowSuggestions(false) }}
                  className="px-4 py-2.5 text-sm text-gray-900 hover:bg-blue-50 cursor-pointer">{c.name}</li>
              ))}
            </ul>
          )}
        ...
        {mode === 'text' && (
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">תוכן הפגישה להדבקה</label>
            <textarea value={textContent} onChange={e => setTextContent(e.target.value)} rows={8}
              placeholder="הדבק כאן פרוטוקול פגישה, הערות, או סיכום חופשי. Claude ינתח ויפיק סיכום מובנה."
        ...
        {mode === 'text' ? (
          <button onClick={submitText} disabled={!clientName.trim() || !textContent.trim()}
            className="w-full bg-blue-600 text-white rounded-2xl py-5 text-lg font-bold disabled:opacity-40 shadow-sm">
            🤖 שלח לניתוח Claude
          </button>
        ) : (
          <button onClick={startRecording} disabled={!clientName.trim()}
